...
class Model:
    def __init__(self):
        self.layers = [
            LayerLinear(in_features=4, out_features=16),  # W_1*x + b < dW , db
            LayerReLU(),
            LayerLinear(in_features=16, out_features=16),  # W_2*x + b
            LayerReLU(),
            LayerLinear(in_features=16, out_features=3),  # W_3*x + b
            LayerSoftmax()
        ]
...
def main():
    ...
    learning_rate = 1e-2
    features, classes = sklearn.datasets.load_iris(return_X_y=True)
