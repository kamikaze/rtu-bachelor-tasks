...
from meeting06.datasets import DatasetFlickrImageHDF5
...
class InceptionNet(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = torch.nn.Conv2d(in_channels=3, out_channels=16, kernel_size=(5, 5), stride=(1, 1), bias=False)
        self.max_pool1 = torch.nn.MaxPool2d(kernel_size=2)
        self.inception_block1 = InceptionBlock(in_channels=16, out_channels=16)

        self.conv2 = torch.nn.Conv2d(in_channels=16 * 4, out_channels=16, kernel_size=(5, 5), stride=(1, 1),
                                     bias=False)
        self.max_pool2 = torch.nn.MaxPool2d(kernel_size=2)
        self.inception_block2 = InceptionBlock(in_channels=16, out_channels=64)
...
def main():
    dataset_full = DatasetFlickrImageHDF5(root='../data', chunks=(BATCH_SIZE, None))
